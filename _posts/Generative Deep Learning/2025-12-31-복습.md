...
<img src="./../images/2025-12-31-%EB%B3%B5%EC%8A%B5/image-20260101174740451.png" alt="image-20260101174740451" style="zoom: 67%;" />

그림에 나온 그래프를 따라서 가중치가 점점 내려온다.
랜덤 위치에서 출발해 가장 로스가 최소가 되는 지점으로 가는것이 목표이다. 즉 저 2차함수의 최소점을 향해 가야한다.
...
![img](./../images/2025-12-31-%EB%B3%B5%EC%8A%B5/img-1767258252615-4.png)
...
수식은 SGD와 유사하다. $v_{t} = \gamma v_{t-1} + \eta \nabla f(w_{t-1})$ 인데, 여기서 $x_t$는 $x_{t-1} - v_t$ 이다.
$v(n)$은 속도를 의미한다. 즉 $v(n-1)$ 이전 위치에서의 속도 즉 관성이고, $\gamma$는 관성 계수가 된다.
...
현재 속도 v에다가 `momentum * 현재 속도 - 학습률 * 그레디언트` 즉, `여태껏 쌓인 관성 * 이번 스텝이 새로 받은 기울기`를 대입하여 바뀐 속도를 찾아낸다. 즉 예전에 가던 방향은 그대로 가되, 새 기울기는 조금만 (즉, 학습률만큼만) 반영하라 라는 뜻이다.
그리고 현재 가중치에 현재 속도 v를 더해주어 가중치를 갱신해준다.

![etc-image-1](./../images/2025-12-31-%EB%B3%B5%EC%8A%B5/img-1767263070325-7.png)
...
class AdaGrad:
    ...
    def update(self, weights, grads):
        ...
        for key in weights.keys():
            ...
            weights[key] -= self.learning_rate * grads[key] / (np.sqrt(self.h[key]) + 1e-7)
```
...
그리고 누적 그레디언트 값에다가 현재 그레디언트의 제곱을 더해준다. 이때 1 - 부식률을 하여 반영하고 싶은 만큼의 양만큼 더해준다.
원래 가중치에서 학습률 * 그레디언트를 h의 제곱근으로 나누어준 값을 빼준다.
...
여기에 역수를 취해서 곱했기 때문에 보정한 m(n)은 원점과 먼 점이 된다.
...
코드를 보면 명확하다.
lr_t에 값을 대입하는 부분을 보면, beta값에다 iter만큼 제곱하는 모습을 볼 수 있다. 이는 beta값을 줄여나가며 학습률을 적용하려는 모습임을 알 수 있다.
...
<img src="./../../images/image-20251224154938271-1767275397649-1.png" alt="image-20251224154938271" style="zoom:50%;" />
...
strides는 필터가 한번에 입력 위를 이동하는 간격을 의미한다. 2이므로 두칸씩 건너뛰며 이동하는 것이다. Stride가 커지면 결과 이미지가 작아질것. 즉 해상도를 줄이는 역할도 한다.

padding은 쉽게 말해 이미지 테두리에 0을 붙이는 것이다. padding=”same”은 입력 데이터를 0으로 패딩해서 strides = 1일 때 출력의 크기를 입력 크기와 동일하게 만든다.

<img src="./../../images/image-20251224160337669-1767275600522-4.png" alt="image-20251224160337669" style="zoom: 67%;" />

이 이미지를 보라. 테두리에 줄 하나를 추가해서 필터링 이후에도 그대로 크기가 유지가 된다. 
`padding = "same"`으로 지정하면 여러 합성곱 층을 통과할 때 텐서의 크기를 쉽게 파악할 수 있다.
이때 합성 곱 층의 출력크기는 $(\frac{height}{stride}, \frac{width}{stride}, 필터개수)$이다.

stride가 늘어나면 넓은 칸을 건너뛰면서 특징을 잡기 때문에 크기가 작아진다.
filters가 커지면 그만큼 많은 합성곱 연산을 하기 때문에 채널의 수가 늘어난다.
padding은 클수록 이미지 상하좌우에 빈칸을 추가하는것이며, same으로 지정하면 크기가 유지된다.  

# 소프트맥스가 뭘까

입력받은 값을 출력으로 0~1 사이값으로 모두 정규화시키키고 모든 출력값들의 합을 항상 1이 되게 하는 함수이다.
