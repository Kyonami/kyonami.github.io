...
![image-20251227224557943](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227224557943.png)
...
![image-20251227225410907](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227225410907.png)

매우 직관적이다. 말그대로 인코더의 역순이다.
2D latent 벡터 z를 입력으로 받아서 Dense로 늘려서 Flatten 이전의 텐서 크기로 복구한다.
이후 Reshape를 통해 형태도 되돌려준다.
Conv2DTranspose를 통해 세 번업 샘플링을 하여 다시 원래 상태로 되돌린다.
...
![image-20251227225758112](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227225758112.png)
...
x = layers.Conv2DTranspose(128, (3, 3), strides=2, activation="relu", padding="same")(x) # Conv2DTranspose 층을 연속으로 쌓음
...
decoder = models.Model(decoder_input, decoder_output)                                    # 케라스 Model 클래스로 디코더 정의. 잠재공간의 임베딩을 통해 원본 이미지 도메인으로 디코딩함.
```
...
![image-20251227231326205](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227231326205.png)
...
![image-20251227231711238](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227231711238.png)

이걸 통해 라벨에 각 색을 부여하여 포인트에 적용하면 아래의 그래프가 나온다.

![image-20251227231742002](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227231742002.png)
...
![image-20251227232029522](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227232029522.png)
...
![image-20251227232436254](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227232436254.png)

이 이미지를 보면 문제가 명확해진다.
일정 간격으로 격자로 디코딩했다.  이때 핵심적인 내용은 아래와 같다.

- 우리가 정의한 공간에서 균일하게 포인트를 고르면 잠재 공간의 가방 영역보다 크기 때문에 앵클 부츠(ID 9)보다 가방 (ID 8)처럼 보이는 것을 디코딩할 가능성이 높다.
...
![image-20251227235018262](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227235018262.png)
...
> $$
> ![image-20251227235146688](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251227235146688.png)
>
> 위 그래프는 평균과 분산 값에 따라 곡선 모양이 어떻게 달라지는지 보여준다. 또한 표준정규분포 는 $\mathcal{N}(0,1)$ 이다.
> 정규분포에서 샘플링은 아래처럼 쓸 수 있다. 여기서 $\epsilon$은 표준정규분포에서 샘플링된다.
> $$
> z = \mu + \sigma \epsilon
> $$
> 
...
> $$
> 이 책에서는 보통 등방성 다변량 정규 분포를 사용한다고 설명한다.
>
...
![image-20251228000016712](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228000016712.png)
...
class Sampling(layers.Layer):	# 케라스의 Layer를 상속해 새로운 층을 만든다.
    def call(self, inputs):	# Layer 상속시 call()을 정의해 새로운 층을 만들 수 있다. 케라스에 없는 변환을 적용하고 싶을때 유용하다.
        z_mean, z_log_var = inputs	# 변이형 오토인코더에선 z_mean, z_log_var로 정의된 정규분포에서 z를 샘플링하기 위해 상속받은 예제이다.
...
z_mean    = layers.Dense(2, name="z_mean")(x)	# Flatten층을 2D 잠재 공간에 바로 연결하는 대신 z_mean, z_log_var층에 연결.
z_log_var = layers.Dense(2, name="z_log_var")(x)
z = Sampling()([z_mean, z_log_var])	# Sampling 층이 파라미터 z_mean, z_log_var로 정의된 정규 분포에서 잠재 공간에 있는 z를 샘플링 함

encoder = models.Model(encoder_input, [z_mean, z_log_var, z], name="encoder")	# 케라스 Model클래스로 인코더 정의. 
```

출력은 아래와 같다.

![image-20251228010448170](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228010448170.png)
...
![image-20251228014609994](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228014609994.png)
...
![image-20251228014836646](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228014836646.png)
...
![image-20251228015912668](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228015912668.png)

훈련엔 레이블이 필요 없지만, 다차원 잠재공간에서 어떻게 특성이 감지되는지 탐색할 때 레이블을 사용 할 것이다.
VAE가 훈련되면 잠재 공간에서 샘플링하여 새로운 유명인 얼굴을 생성할 수 있다.

```python
train_data = utils.image_dataset_from_directory(	# 이미지가 저장된 디렉터리에서 텐서플로우 데이터 셋을 생성하는 함수
...
```python
def preprocess(img):
    img = tf.cast(img, "float32") / 255.0	# 원본 픽셀 값은 0~255 사이이다. 0~1로 스케일링 해준다.
    return img
...
![image-20251228020847226](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228020847226.png)

대략 5회 에폭정도 진행하면 VAE가 새로운 유명인의 얼굴을 그릴수 있다.

## 3.4.3 VAE 분석

![image-20251228021102627](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228021102627.png)
...
![image-20251228021421217](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228021421217.png)
...
z_sample = np.random.normal(size=(grid_width * grid_height, 200))  # 200차원의 잠재공간에서 표준 다변량 정규분포로부터 30개의 점을 샘플링한다.
...
![image-20251228021717849](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228021717849.png)
...
![image-20251228022658745](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228022658745.png)
...
![image-20251228023148394](./../images/2025-12-27-Chapter%203.%EB%B3%80%EC%9D%B4%ED%98%95%20%EC%98%A4%ED%86%A0%EC%9D%B8%EC%BD%94%EB%8D%94/image-20251228023148394.png)
